...
def test_plan_command_writes_validated_plan(tmp_path: Path, monkeypatch):
    ...
    class FakePlanner:
        ...
        def plan(self, goal):
            ...
            return ProjectPlan(project_name="demo", tasks=[TaskSpec(id="a", objective="A", target_path="a.py")])

    monkeypatch.setattr(cli, "OllamaPlanner", FakePlanner)
    output = tmp_path / "plan.json"
    code = cli.main(["plan", "--goal", "build demo", "--model", "qwen3:8b", "--output", str(output)])
    assert code == 0
...
def test_run_command_loads_plan_and_reports_status(tmp_path: Path, monkeypatch, capsys):
    ...
    plan_path.write_text(ProjectPlan(project_name="demo", tasks=[TaskSpec(id="a", objective="A", target_path="a.py")]).model_dump_json())
    ...
    code = cli.main([
        "run", "--plan", str(plan_path), "--model", "qwen3:8b",
        "--workspace", str(tmp_path / "work"), "--db", str(tmp_path / "state.db")
    ])
    assert code == 0
